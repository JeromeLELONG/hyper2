                ...
                p => {
                    ...
                    if ((this.dateDebut && this.dateFin) && (this.dateDebut < this.dateFin)) {
                        ...
                        while (moment(startDate).format('MM/D/YYYY') !== moment(endDate).format('MM/D/YYYY')) {
                            ...
                            for (let i = 0, len = this.selectedSalles.length; i < len; i++) {
                                let totalPresents = 0;
                                const salle = this.selectedSalles[i];
                                const jour = moment(startDate).format('DD/MM/YYYY')
                                totalPresents = p.filter(function (item) {
                                    return item.codesalle === salle;
                                }, salle).filter(function (item) {
                                    return item.datesem === jour;
                                }, jour).reduce(function (a, b) { return a + +b.nbpresents; }, 0);
