import {
    NgModule, Component, OnInit, style, animate, transition, state, trigger, ViewChild,
    group, AnimationTransitionEvent, Inject
} from '@angular/core';
...
export class EditionFichesComponent implements OnInit {
    ...
    getCours() {
        ...
        this.coursService.getRessourcesWithDateAndGroup(datestring, this.selectedType, this.trancheHoraire, 'VP')
            .subscribe((ressourcesVP: Cours[]) => {
                this.ressources = ressourcesVP
                this.coursService.getRessourcesWithDateAndGroup(datestring, this.selectedType, this.trancheHoraire, 'einte')
                    .subscribe((ressourcesEnceintes: Cours[]) => {
                        this.ressources = this.ressources.concat(ressourcesEnceintes);
                        this.coursService.getRessourcesWithDateAndGroup(datestring, this.selectedType, this.trancheHoraire, 'projecteur')
                            .subscribe((ressourcesProjecteur: Cours[]) => {
                                this.ressources = this.ressources.concat(ressourcesProjecteur);
    ...
    getMateriel(numero: string) {
        const identifiant = numero.split(".")[0] + '.' + numero.split(".")[1];
        const ressource = this.ressources.find(x => (x.numero.split(".")[0] + '.' + x.numero.split(".")[1]) === identifiant);
        if (ressource)
    ...
    lancerProgressEdition() {
        const queryArr = JSON.parse(JSON.stringify(this.lot));
        this.loading = true;
        this.failed = false;
    ...
    makeRequest(queryArr, previousObservable = null) {
        if (queryArr.length) {
            const payload = JSON.stringify(queryArr[0]);
            const headers = new Headers();
            const lot = queryArr[0];
            queryArr.splice(0, 1);
    ...
    private handleError(error: any) {
        ...
        const errorMsg = error.message || `Malaise! le webservice ne peut être joint`
